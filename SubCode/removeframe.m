...
input_im=im2double(input_im);%3
[a,b,c]=size(input_im);
if c~=3
        gray=im2double(input_im);
    else
        gray=im2double(rgb2gray(input_im));
end
...
% we assume that there exists a frame when one more lines parallel to the
% image side are detected 
if flagrm>1 
